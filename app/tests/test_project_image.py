...
class ProjectImagePermission(TestCase):
    
    @staticmethod
    def perform_create():
        return models.ProjectImage.objects.create(
            name="delete_me"
        )
    ...
    def test_unauthenticated_users_cant_create(self):
        response = self.unauthorized_client.get(reverse_lazy(conf.PROJECTIMAGE_CREATE_URL_NAME))
        self.assertNotEqual(response.status_code, 200)

    def test_unauthenticated_users_cant_update(self):
        response = self.unauthorized_client.get(
            reverse_lazy(
                conf.PROJECTIMAGE_UPDATE_URL_NAME,
    ...
    def test_unauthenticated_users_cant_delete(self):
        response = self.unauthorized_client.get(
            reverse_lazy(
                conf.PROJECTIMAGE_DELETE_URL_NAME,
                kwargs=self.args_for_reverse_lazy()
            )
        )
        self.assertNotEqual(response.status_code, 200)
    ...
    def test_unauthorized_users_cant_create(self):
        response = self.client.get(reverse_lazy(conf.PROJECTIMAGE_CREATE_URL_NAME))
        self.assertNotEqual(response.status_code, 200)
    ...
    def test_authorized_users_can_create(self):
        create_permission = auth_models.Permission.objects.get(codename="add_projectimage")
        self.user.user_permissions.add(create_permission)
        response = self.client.get(reverse_lazy(conf.PROJECTIMAGE_CREATE_URL_NAME))
        self.assertEqual(response.status_code, 200)

    def test_authorized_users_can_update(self):
        change_permission = auth_models.Permission.objects.get(codename="change_projectimage")
    ...
    def test_authorized_users_can_delete(self):
        delete_permission = auth_models.Permission.objects.get(codename="delete_projectimage")
